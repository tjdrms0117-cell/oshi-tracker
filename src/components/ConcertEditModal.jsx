...
import { X, Save, Plus, Calendar, MapPin, Ticket, Image, Upload, Link, Users } from 'lucide-react'
import { 
  fetchVenues, 
  fetchArtists,
  updateConcert, 
...
export default function ConcertEditModal({ concertId, onClose, onDone }) {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [c, artistsData] = await Promise.all([
      ...
      // 현재 등록된 아티스트 ID 세팅
      const currentArtistIds = (c.concert_artists || [])
        .map(ca => ca.artist?.id)
        .filter(Boolean)
      // concert_artists 없으면 기본 artist_id fallback
      setSelectedArtistIds(
        currentArtistIds.length > 0 ? currentArtistIds : (c.artist_id ? [c.artist_id] : [])
      )
  ...
  const filteredArtists = allArtists.filter(a =>
    !artistSearch || a.name.toLowerCase().includes(artistSearch.toLowerCase()) || (a.name_jp || '').toLowerCase().includes(artistSearch.toLowerCase())
  )
  ...
  const handleSave = async () => {
    ...
    try {
      await updateConcert(concertId, {
        ...
        // artist_id는 첫 번째 선택 아티스트로
        artist_id: selectedArtistIds[0] || concert.artist_id,
      })

      // concert_artists 업데이트
      await updateConcertArtists(concertId, selectedArtistIds)
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center p-0 md:p-5" onClick={onClose}>
      <div className="bg-white dark:bg-zinc-900 rounded-t-2xl md:rounded-2xl w-full max-w-2xl max-h-[95vh] md:max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {/* 아티스트 선택 */}
          <Section icon={Users} title={`아티스트 (${selectedArtistIds.length}명 선택)`}>
            <div className="space-y-2">
              {/* 선택된 아티스트 태그 */}
              {selectedArtistIds.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {selectedArtistIds.map(id => {
                    ...
                    if (!a) return null
                    return (
                      <span key={id} className="flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-full text-white" style={{ background: a.color || '#888' }}>
                        {a.name}
                        <button onClick={() => toggleArtist(id)} className="ml-0.5 opacity-70 hover:opacity-100">
              ...
              />
              {/* 아티스트 목록 */}
              <div className="max-h-32 overflow-y-auto space-y-0.5 rounded-lg border border-stone-200 dark:border-zinc-700">
                {filteredArtists.map(a => (
                  ...
                  >
                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: a.color || '#888' }} />
                    <span className="font-medium text-zinc-800 dark:text-zinc-200">{a.name}</span>
                    {a.name_jp && <span className="text-zinc-400 text-[10px]">{a.name_jp}</span>}
                    {selectedArtistIds.includes(a.id) && <span className="ml-auto text-pink-500 text-[10px] font-bold">✓</span>}
                  </button>
                ))}
              </div>
            </div>
